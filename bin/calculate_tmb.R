...
option_list = list(
  make_option(c("-m", "--maf"), type="character", default=NULL,
              help="Input annotated variant MAF file"),
  make_option(c("-t", "--hotspot_maf"), type="character", default=NULL,
              help="Path to the hotspot variant MAF file"),
  make_option(c("-o", "--out"), type="character", default="tmb.tsv",
              help="The name of the output TSV file  [default= %default]"),
  make_option(c("-d", "--depth"), type="integer", default=250,
              help="The sequence coverage of the tumor"),
  make_option(c("-c", "--count"), type="integer", default=5,
              help="Alternative allele count in tumors"),
  make_option(c("-v", "--vaf"), type="double", default=0.05,
              help="Frequency of variant alleles in tumors"),
  make_option(c("-s", "--callable"), type="integer", default=1408279,
              help="Designated sequencing regions for TMB calculation")
); 
...
calculate_tmb <- function(maf_file, hotspot_maf, out_tsv, tumor_coverage, alt_count, tumor_vaf, callable) {
  ...
  # read the raw annotated variants from the input MAF file
  tmb_maf <- read.delim(maf_file, header = TRUE, sep = "\t", comment.char = "#", stringsAsFactors = FALSE)
  # choose variants of interest
  tmb_df <- tmb_maf %>%
