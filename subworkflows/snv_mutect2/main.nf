...
workflow SNV_MUTECT2 {
    ...
    GATK4_FILTERMUTECTCALLS ( GATK4_MUTECT2.out.vcf.combine(GATK4_MUTECT2.out.tbi,
                                by:[0,1]).combine(GATK4_MUTECT2.out.stats, by:[0,1]),
                                GATK4_CALCULATECONTAMINATION.out.contamination,
                                ch_fasta, ch_fai, ch_dict)
    
    BCFTOOLS_NORM ( GATK4_FILTERMUTECTCALLS.out.vcf.combine(GATK4_FILTERMUTECTCALLS.out.tbi, by:[0,1]), ch_fasta )
    ch_versions = ch_versions.mix(BCFTOOLS_NORM.out.versions)

    ch_vcf_files = BCFTOOLS_NORM
                        .out.vcf
                        .map { meta, norm_vcfs ->
                            new_meta = [id:meta.pid, sid:meta.id]
                            [new_meta, norm_vcfs] }
                        .groupTuple()
